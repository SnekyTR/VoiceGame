...
namespace WarriorAnimsFREE
{
	public class WarriorController:SuperStateMachine
	{
		...
		[HideInInspector] public bool inputWeaponWith;
			...
			if (allowedInput) {
				...
				if (!useInputSystem) {
					if (warriorInputController != null) {
						...
						inputRoll = warriorInputController.inputRoll;
						inputWeaponWith = warriorInputController.inputWeaponWith;
					}
		...
		private void Update()
		{
			...
			if (MaintainingGround() && canAction) { Attacking();  Rolling(); Equipping(); }
		...
		{
			if (inputAttack) { Attack1(); weaponAttack.damageTwo = true; StartCoroutine(weaponAttack.DamageTwo()); }
		}
		...
		public void Roll()
        {
			StartCoroutine(CouRoll());
			SetAnimatorBool("roll", true);
		}
		IEnumerator CouRoll()
        {
			yield return new WaitForSeconds(1f);
			SetAnimatorBool("roll", false);
		}
		private void Equipping()
        {
			
            if (inputWeaponWith && !equiped) {
				Debug.Log("Equipa");
				SetAnimatorBool("sheathing", true);
				
				StartCoroutine(Equip());
			}
			else if(inputWeaponWith && equiped)
            {
				Debug.Log("DesEquipa");
				SetAnimatorBool("withdraw", true);
		...
		IEnumerator Equip()
        {
			if (!equiped)
            {
				...
				SetAnimatorBool("sheathing", false);
				equiped = true;
				yield return new WaitForSeconds(1f);
				...
				yield return new WaitForSeconds(0.1f);
				SetAnimatorBool("withdraw", false);
				equiped = false;
